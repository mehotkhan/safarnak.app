  ...
  resolvers: {
    Query: {
      getMessages: async (
        _parent: unknown,
        _args: unknown,
        context: DefaultPublishableContext<Env>
      ) => {
    ...
    Mutation: {
      addMessage: async (
        ...
        _context: DefaultPublishableContext<Env>
      ) => {
        const db = drizzle(_context.env.DB);
        ...
        _context.publish('NEW_MESSAGES', { newMessages: newMsg });
